...
Up to 1000 items per request. Ingestion is idempotent on `(source, external_id)`: a re-pushed row never mutates the stored copy and reports `replayed`. Invalid rows report `error` per item without blocking the rest. Amounts are integer minor units. `raw` retains the provider payload for audit.

`reference_rail`, `reference_kind`, and `reference_value` must be provided together. When the triple resolves to a [transaction external ref](/ledger/api-reference/introduction), matching pairs that transaction's posting first, exactly, with strategy `external_ref`. Write the same triple on both sides when you book the internal transaction and reconciliation stops depending on amount heuristics.
...
```bash
curl "https://ledger.southpay.io/api/v1/external_transactions?status=open&source_id={id}" \
  -H "Authorization: Bearer $TOKEN"
```
...
```bash
curl -X POST "https://ledger.southpay.io/api/v1/sources/{id}/inbound" \
  -H "Authorization: Bearer $TOKEN"
```
...
curl -X POST "$INBOUND_URL" \
  -H "Content-Type: application/json" \
  -H "Southpay-Inbound-Signature: t=$T,v1=$SIG" \
  -d "$BODY"
```
