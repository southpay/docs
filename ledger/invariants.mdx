---
title: "Invariants"
description: "A ledger is not a set of rules we promise to follow. It is a system built so that certain financial states cannot exist. Each invariant makes a class of corruption impossible."
---
...
Every amount is a `BIGINT` in the currency's smallest unit (cents, satoshis); floats are rejected at the schema. An account's `currency` is set at creation and can never change.
