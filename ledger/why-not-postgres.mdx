...
description: "Postgres is the right tool. The trap is assuming a ledger is a schema problem. It is a distributed correctness problem, and getting it wrong corrupts money silently."
---

This is a fair question, and the honest answer starts by agreeing. Postgres is an excellent foundation for a ledger. Southpay Ledger runs on Postgres. So this page is not "avoid Postgres". It is about what you are actually signing up to build when you decide to roll your own ledger on top of it, and why that is more than it looks.
...
<Info>
  Once money moves between more than one account, or crosses a system boundary (an API retry, an external processor, a cached balance), you are in ledger territory. Correctness now lives across transactions, across processes, and across time, which is exactly where a single database transaction stops protecting you.
</Info>
...
The strongest objection to this page is true: Postgres gives you ACID, plus constraints, plus careful design. You should use all of it. But ACID's guarantees apply to a single database transaction, and financial correctness lives in the gaps ACID was never meant to cover.
...
These are the production failures, and what makes them dangerous is that every one of them returns `200 OK` at the time.
