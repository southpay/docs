...
| Refund a payment | Blocked with `403 mandate_required` |
| Enable or disable an accepted token | Blocked with `403 mandate_required` |
| Create a payout | Blocked with `403 payout_controls_required` until a spend limit exists for the asset |
| Issue a session mandate, settle an x402 charge | Blocked with `403 mandate_required` |
...
An agent sees only its own records. Every agentic query is filtered by the calling credential, so payments, payouts, refunds and settlements created by one credential are invisible to another on the same store. An agent also cannot manage credentials, neither its own nor any other.
...
Agentic endpoints allow **600 requests per minute per credential** in live mode, with a burst of 100, returning `429` with code `rate_limited` when exceeded. Every `429` carries a `Retry-After` header in seconds. The budget is shared across every agentic path, so payments, payouts and refunds all draw from it.

`GET /api/v2/agentic/market_data` is the exception: it is deliberately unauthenticated and limited to 30 requests per minute per IP. See [Rate limits](/rate-limits) for every throttled path on the API.
