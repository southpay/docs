...
description: "Capped spending envelopes for pay-per-call commerce. Both prerequisites are off by default: mandates need a provider, dispatch needs a flag."
...
- the store has the `x402_onchain_dispatch` feature flag enabled — it is off unless staff turn it on;
...
<Warning>
  Polling a `submitted` settlement for `confirmed` on a store without `x402_onchain_dispatch` is an infinite loop. Check for the flag with your SouthPay contact before you build a state machine that waits on it.
</Warning>
...
`Idempotency-Key` is required; without it the request is refused with `422 idempotency_key_required`. Retrying with the same key returns the original settlement, rendered with the controller's normal `201`.
...
| `confirmed` | On-chain transfer confirmed. Only reachable with `x402_onchain_dispatch` enabled, in live mode |
| `failed` | Dispatch failed; `failure_reason` is set. Same preconditions |

`confirmed` and `failed` are terminal. Without the dispatch flag a settlement finishes its life as `submitted`, which is the state most integrations will see today.

If dispatch is enabled for your store, `tx_ref` carries the provider transaction reference once broadcast, and dispatch is keyed deterministically per settlement so retries and concurrent workers cannot double-send.
