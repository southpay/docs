...
- on-chain dispatch is enabled for the store, which is off unless SouthPay staff turn it on;
- the settlement is in **live mode**, since test-mode settlements are never dispatched;
- `amount_atomic` is non-zero;
- `pay_to` is set, since a settlement without one is failed with reason `missing_pay_to`.

With dispatch off, a settlement stays `submitted` indefinitely and `tx_ref` stays `null`. The spend is real on SouthPay's internal ledger, reducing the store balance and the mandate envelope, but no on-chain transfer exists. Do not treat `submitted` as settlement finality, and do not release goods against it.
