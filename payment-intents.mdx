...
A Payment Intent represents a single, atomic payment request. You specify a fiat amount and currency; Southpay calculates the equivalent crypto amount at the current exchange rate and allocates a deposit address for each crypto asset your store has enabled.

The intent captures a rate snapshot at creation time, so the customer always knows the exact crypto amount to send. Which tokens appear on the intent depends on what your store enabled, not on the request. See [Supported currencies and assets](/currencies) for the full catalog and how this works.
...
| `currency` | string | Yes | ISO 4217 fiat code. One of `USD`, `EUR`, `GBP`, `HKD`, `SGD`, `JPY`, `DKK`, `CHF`, `SEK`, `NOK`. See [supported currencies](/currencies). |
...
stateDiagram-v2
  ...
  pending --> refunded : refund confirmed

  processing --> completed : confirmations reached
  processing --> failed : underpayment confirmed
  processing --> failed : overpayment rejected
  processing --> expired : window elapsed, no full payment
  processing --> refunded : refund confirmed

  completed --> refunded : refund confirmed
  expired --> refunded : refund confirmed
  failed --> refunded : refund confirmed
...
Every status transition creates a `PaymentEvent` record. You can retrieve the full history for any intent. Event names match the [webhook event types](/webhooks) exactly, so you can reconcile this log against the deliveries you received.
